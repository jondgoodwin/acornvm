...
#define isualpha(c) (c>0xA0 || isalpha(c))

/** Algorithm for determining if character is a digit 0-9 */
#define isudigit(c) (c>='0' && c<='9')
...
Value newLex(Value th, Value *dest, Value src, Value url) {
	...
	*dest = (Value) lex;
	...
	lex->bytepos = 
		lex->linebeg = getSize(src)>=3 && 0==strncmp("\xEF\xBB\xBF", toStr(src), 3)? 3 : 0;
	lex->linenbr = 1;
...
Auchar lex_thischar(LexInfo* lex) {
...
/** Return the current unicode character whose UTF-8 bytes start at lex->bytepos */
Auchar lex_nextchar(LexInfo* lex) {
	const char *src = &toStr(lex->source)[lex->bytepos];
	int nbytes;
	Auchar chr;
